...
\maketitle
\begin{abstract}
In this essay, we discuss the question of whether machines can have minds. We start off by introducing some of the arguments and ideas that have been proposed on the subject during the last 70 years. We then try to define what a mind is, showing some examples from the animal kingdom. We move on to describe some different methods of testing if something has a mind and discuss what the tests actually show, before considering a few examples of how simple minds can arise. We finally conclude that although human brains and modern computers are fundamentally different, the possibility of a machine mind seems plausible. However, a potential machine mind will most likely be very different from a human mind and thus make it impossible for machines and humans to fully understand each other.
%\todo{possible keywords: mind, strong AI, intelligence, artificial general intelligence}
\end{abstract}

\section{Introduction}
\label{sec:introduction}

It all started with the question "Can machines think?". We as human civilisation have been creating tools to solve problems since the beginning of our time. As we grew better at creating tools and collecting them together, we were able to build more and more complex machines upon each other. Then came the computer, a machine first designed as a computational tool, which we later gave the ability to improve itself\todo{I don't know what the authors mean but I don't recall this ever happening}. Inevitably, we started to feel the uncertainty of what this can lead to.
...
\todo{It’s more important to get the definition of minds, machines, and intelligence than comparing strong and weak AI in the introduction}
...
\todo{the titles used for strong and weak AI are quite long that they’re more like sentences than titles.}
...
However, the general consensus is that general intelligence must accomplish the following: \todo{This stick to me as not supported, because it could be debated.} \todo{the "general consensus" that general intelligence must accomplish the given set of 5 skills is also debatable and probably requires a citation to support}
\begin{enumerate}
\item reasoning
\item learning
\item planning
...
\todo{a recommendation would be to switch 2.2 and 2.1, so that the authors would first explain how the mind would arise in biological as well as artificial beings.}
...
\todo{This section is written very badly especially when compared to the previous two sections.}
...
Brains are too complex to recreate or mimic at the time of writing. Yet, if it is possible to show, our direction of work is on the right track\todo{This sentence makes no sense, for example.}. The examples shown in Section \ref{sec:simpleminds} can maybe offer us some confidence. One can either build very simple units (replicating slime mould or neurons) or assemble meta-complex brains like bee's. Given previous studies on composition, or to say components, of human mind, it could be possible even with smaller scale of minds, like those of bees, can lead us to some association of certain functions of mind to its corresponding circuitry. Or by focusing on single simple cell replications, like slime mould, and grouping large quantities of these simple agents for performing complex behaviour in order to help us understand how to transform basic communication of individual agents into higher level patterns which in turn trigger complex behaviours. Although there exists technological limitations, for instance a maximum computation power per single chip before causing meltdown on chip itself, we can still combine many known principles from the development of modern computer systems in attempts to explore more possibilities. 
...
%evolution curve of machine must exceed ours. 

\subsection{Final words} \todo{The last words could be expanded or debated.}
